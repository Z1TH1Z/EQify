...
def get_response_from_gemini(user_message):
    """Get response from Gemini AI based on user message."""
    try:
...
def send_message():
    """Handle sending a message and getting a response from Gemini AI."""
    user_message = f"Get me the lyrics of '{track_name}' by {track_artists}."
...
send_button = tk.Button(root, text="Get Lyrics", command=send_message)
